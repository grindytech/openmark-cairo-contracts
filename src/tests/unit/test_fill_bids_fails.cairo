...
fn fill_bids_invalid_signature_len_panics() {
    ...
    openmark.fill_bids(bids.span(), nft_token, tokenIds, unitPrice);
}
#[test]
#[available_gas(2000000)]
...
fn fill_bids_too_many_bids_panics() {
    ...
    openmark.validate_bids(bids.span(), seller, nft_token, tokenIds, unitPrice);
}
...
fn fill_bids_no_bids_panics() {
    let (_, openmark_address, nft_token, payment_token, seller, _, tokenIds, unitPrice) =
        create_bids();
...
fn fill_bids_asking_price_too_high_panics() {
    ...
    openmark.validate_bids(signed_bids, seller, nft_token, tokenIds, unitPrice + 1);
}
...
fn fill_bids_sig_expired_panics() {
    ...
    openmark.validate_bids(signed_bids, seller, nft_token, tokenIds, unitPrice);
}
...
fn fill_bids_nft_mismatch_panics() {
    let (mut signed_bids, openmark_address, _, payment_token, seller, _, tokenIds, unitPrice) =
        create_bids();
    ...
    openmark.validate_bids(signed_bids, seller, payment_token, tokenIds, unitPrice);
}
...
fn fill_bids_exceed_bid_nfts_panics() {
    let (mut signed_bids, openmark_address, _, payment_token, seller, _, _, _) = create_bids();
